...
class MeasurementCompare():
	...
	def checkHumidityLimits(self):
		...
		if float(self.measuredHumidity) < float(self.humidityLowLimit):
			self.limitCheckWarningMsg = self.limitCheckWarningMsg + "ALERTA: Humedad baja en el sensor: {0}\nTemperatura: {1}\nLimite configurado de Humedad: {2}\nHumedad: {3}\n".format(self.sensor,self.measuredTemperature,self.humidityLowLimit,self.measuredHumidity)
			self.betweenLimits = False
	...
	def checkTemperatureChange(self):
		...
		measurementUnit = "Grados centigrados"
		...
		if self._isFloat(self.previousTemperature):
			...
			if self.measuredTemperature > float(self.previousTemperature) + self.temperatureThreshold:
				...
				self.warningMsg = "ALERTA: La temperatura incremento mas de  {0} {1} en el sensor {2}\nTemperatura actual: {3} {1}\nTemperatura anterior: {4} {1}".format(self.temperatureThreshold,measurementUnit,self.sensor,self.measuredTemperature,self.previousTemperature)
			...
			if self.measuredTemperature < float(self.previousTemperature) - self.temperatureThreshold:
				...
				self.sendWarning = True
				# Compose warning message to be sended out
				self.warningMsg = "ALRTA: La temperatura bajo a {0} {1} en el sensor {2}\nTemperatura actual: {3} {1}\nTemperatura anterior: {4} {1}".format(self.temperatureThreshold,measurementUnit,self.sensor,self.measuredTemperature,self.previousTemperature)
	...
	def checkHumidityChange(self):
		...
		if self._isFloat(self.previousHumidity):
			...
			if self.measuredHumidity > float(self.previousHumidity) + self.humidityThreshold:
				...
				self.warningMsg = self.warningMsg + "ALERTA: La Humedad incremento {0} porciento en el sensor {1}\nHumedad actual: {2} \nHumedad previa: {3}".format(self.humidityThreshold,self.sensor,self.measuredHumidity,self.previousHumidity)
			...
			if self.measuredHumidity < float(self.previousHumidity) - self.humidityThreshold:
				...
				# Compose warning message to be sended out.
				# Note that this will add new warningmessage after first in case that also temperature comparisons caused warning to be sent
				self.warningMsg = self.warningMsg + "ALERTA: La Humedad bajo {0} porciento en el sensor {1}\nHumedad actual: {2} \nHumedad previa: {3}".format(self.humidityThreshold,self.sensor,self.measuredHumidity,self.previousHumidity)
